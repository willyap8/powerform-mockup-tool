// ---------------------------------------------------------------------------
// panels.jsx — Right-side history panel, properties panel, print preview,
// context menu, JSON view.
// ---------------------------------------------------------------------------
              ...
              <div key={i} style={{
                ...
                {isPreviewing && (
                  <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                    <button onClick={onConfirmRollback} style={{
                      ...panelBtn(false), background: '#16a34a', color: '#fff', borderColor: '#16a34a', fontSize: 11,
...
// ---------------------------------------------------------------------------
// DeferredInput — value updates live (so the canvas reflects typing) but only
// commits a history entry on blur or Enter, when the value has actually
// changed since focus.
// ---------------------------------------------------------------------------
function DeferredInput({ value, onLive, onCommitDesc, multiline, style, ...rest }) {
  const valueAtFocusRef = useRefP(value);
  const handleFocus = (e) => { valueAtFocusRef.current = e.target.value; };
  const handleBlur = (e) => {
    if (e.target.value !== valueAtFocusRef.current) onCommitDesc(e.target.value);
  };
  const handleKey = (e) => {
...
// ---------------------------------------------------------------------------
// Properties panel — appears when exactly one block is selected in Edit Layout
// ---------------------------------------------------------------------------
function PropertiesPanel({ block, onLiveUpdate, onCommitDesc, onApplyAndCommit, onClose, historyCollapsed }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 600, fontSize: 12, color: '#111827' }}>
          Properties
          <div style={{ fontWeight: 500, fontSize: 10, color: '#9ca3af', marginTop: 2 }}>
            {describeBlock(block)}
          </div>
        </div>
      ...
      <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <Row label="Position">
          <DeferredNumberField
            value={block.x}
            onLive={(v) => onLiveUpdate({ x: v })}
            onCommitDesc={() => onCommitDesc('Moved ' + describeBlock(block))}
            suffix="x"
          />
          <DeferredNumberField
            value={block.y}
            onLive={(v) => onLiveUpdate({ y: v })}
            onCommitDesc={() => onCommitDesc('Moved ' + describeBlock(block))}
            suffix="y"
          />
        </Row>
        ...
        {isField && (
          <Row label="Size">
            <DeferredNumberField
              value={block.width}
              onLive={(v) => onLiveUpdate({ width: v })}
              onCommitDesc={() => onCommitDesc('Resized ' + describeBlock(block))}
              suffix="w"
            />
            <DeferredNumberField
              value={block.height}
              onLive={(v) => onLiveUpdate({ height: v })}
              onCommitDesc={() => onCommitDesc('Resized ' + describeBlock(block))}
              suffix="h"
            />
          </Row>
        ...
        {isText && (
          <Row label="Width">
            <DeferredNumberField
              ...
              onLive={(v) => onLiveUpdate({ width: v })}
              onCommitDesc={() => onCommitDesc('Resized text')}
              suffix="w"
            />
          </Row>
        )}

        {/* Label (not for checkbox / radio groups, which have no group title) */}
        ...
        {isText && (
          <Row label="Variant">
            <select
              value={block.variant}
              onChange={(e) => onApplyAndCommit({ variant: e.target.value }, 'Changed text variant')}
              style={inputSt}
            >
        ...
        {isField && (
          <Row label="Field type">
            <select
              ...
              style={inputSt}
            >
              {['text','number','date','time','checkbox','radio','textarea'].map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </Row>
        )}

        {/* Placeholder (skip for checkbox / radio group) */}
        ...
        {/* Mandatory (skip for checkbox / radio group) */}
        {isField && !isGroupField && (
          <Row label="">
        ...
        {/* Checkbox group: editable list of options */}
        {isCheckboxGroup && (
        ...
        {/* Radio group: editable list of options with a single "preselected" pick */}
        {isRadioGroup && (
...
// ---------------------------------------------------------------------------
// Checkbox-options editor: list of labels with add/remove + per-row edit
// ---------------------------------------------------------------------------
function CheckboxOptionsEditor({ options, onLive, onCommitDesc }) {
  // Local controlled list mirrors props but lets us defer history commits.
  return (
        ...
        <div key={i} style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          <button
            onClick={() => {
              ...
              onLive(next);
              onCommitDesc("Removed checkbox '" + opt + "'");
            }}
...
// ---------------------------------------------------------------------------
// Radio-options editor: list of labels + a single preselected pick (or none).
// Tap the circle on a row to make it the default; tap again to clear.
// ---------------------------------------------------------------------------
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            <button
              onClick={() => {
                const next = options.slice(); next.splice(i, 1);
                // Adjust preselected index if needed
...
// ---------------------------------------------------------------------------
// openPrintWindow — opens a new browser tab/window containing just the form
// rendered as static HTML. User can Cmd/Ctrl+P from that window.
// ---------------------------------------------------------------------------
function openPrintWindow(form, options) {
  ...
  // Block geometry — match getBlockRect() from constants.js so the popup
  // measures the form exactly the same way the live canvas does.
  function rectFor(b) {
    ...
    if (b.type === 'field') {
      const w = b.width || FIELD_DEFAULTS[b.fieldType].width;
      const h = b.height || FIELD_DEFAULTS[b.fieldType].height;
      if (b.fieldType === 'checkbox') return { x: b.x, y: b.y, w, h };
      return { x: b.x, y: b.y - FIELD_LABEL_HEIGHT, w, h: h + FIELD_LABEL_HEIGHT };
    }
  ...
  function renderBlock(b) {
    ...
    if (b.type === 'field') {
      ...
      const w = b.width || FIELD_DEFAULTS[ft].width;
      const h = b.height || FIELD_DEFAULTS[ft].height;
      const showMandatoryBg = !!b.mandatory;
      const inputBg = showMandatoryBg ? MANDATORY_BG : '#fff';
      ...
      if (ft === 'checkbox' || ft === 'radio') {
        ...
        const opts = (b.options || ['']).map((label, i) => {
          if (isRadio) {
            ...
            return `
              <span style="display:inline-flex;align-items:center;gap:4px;font:${INPUT_FONT};color:${NAVY};line-height:16px;white-space:nowrap;">
                <span style="display:inline-flex;align-items:center;justify-content:center;width:11px;height:11px;border-radius:50%;border:1px solid #555;background:#fff;box-sizing:border-box;">${isChecked ? `<span style="display:block;width:5px;height:5px;border-radius:50%;background:${NAVY};"></span>` : ''}</span>
                <span>${esc(label)}</span>
              </span>`;
          }
          return `
            <span style="display:inline-flex;align-items:center;gap:4px;font:${INPUT_FONT};color:${NAVY};line-height:16px;white-space:nowrap;">
              <span style="display:inline-block;width:10px;height:10px;border:1px solid #555;background:#fff;"></span>
              <span>${esc(label)}</span>
            </span>`;
        }).join('');
        return `<div style="${pos}width:${w}px;height:${h}px;box-sizing:border-box;padding:2px 4px;background:#fff;border:1px solid transparent;display:flex;flex-wrap:wrap;align-content:flex-start;gap:4px 12px;overflow:hidden;">${opts}</div>`;
      }
      ...
      const inputBoxStyle = `width:${w}px;height:${h}px;border:1px solid ${INPUT_BORDER};background:${inputBg};font:${INPUT_FONT};color:${NAVY};padding:${ft === 'textarea' ? '3px 4px' : '0 4px'};box-sizing:border-box;`;
      const inputHtml = `<div style="${inputBoxStyle}">${ft === 'textarea' ? '' : ''}</div>`;
      return `<div style="${pos}">${labelHtml}${inputHtml}</div>`;
    }
    if (b.type === 'sticky') {
      if (!includeNotes) return '';
      return `<div style="${pos}width:${r.w}px;height:${r.h}px;background:#fff59d;border:1px solid #e6cc44;box-shadow:0 1px 2px rgba(0,0,0,0.08);box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;">
        <div style="height:16px;background:#fbe57c;border-bottom:1px solid #e6cc44;padding:0 6px;font:bold 9px Tahoma,sans-serif;color:#7a6a1a;letter-spacing:0.4px;text-transform:uppercase;display:flex;align-items:center;">Configuration note</div>
        <div style="flex:1;padding:4px 6px;font:11px/1.4 Tahoma,sans-serif;color:#3f3a1a;white-space:pre-wrap;word-break:break-word;overflow:hidden;">${esc(b.text || '')}</div>
      </div>`;
    }
    return '';
  }
...
<style>
  ...
  body { font-family: system-ui, -apple-system, "Segoe UI", Helvetica, sans-serif; color: #111827; }
  .toolbar {
...
// ---------------------------------------------------------------------------
// PrintOptionsDialog — small modal asking 'include notes?' then opens
// the actual print window via openPrintWindow().
// ---------------------------------------------------------------------------
function PrintOptionsDialog({ form, onClose }) {
  const [includeNotes, setIncludeNotes] = useStateP(false);
          ...
          <label style={{
            ...
            <span>
              ...
              <span style={{ fontSize: 12, color: '#6b7280' }}>
                {hasNotes
                  ? 'Show configuration notes on the printed form.'
                  : 'No sticky notes in the current mockup.'}
              </span>
            </span>
          </label>
        </div>
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: 'rgb(0, 48, 135)', color: '#fff', border: 'none',
...
Object.assign(window, {
  HistoryPanel, PropertiesPanel, ContextMenu, panelBtn,
  openPrintWindow, PrintOptionsDialog,
  NotesPane,
});

// ---------------------------------------------------------------------------
// NotesPane — left-docked notes editor. Persistent across sessions and
// saved/imported alongside the PowerForm. Hard-capped at 32,000 chars.
// "Save" button commits the draft to the form's notes (unsaved-indicator dot).
// ---------------------------------------------------------------------------
const NOTES_CHAR_LIMIT = 32000;

function NotesPane({ committed, onSave, onClose }) {
  const [draft, setDraft] = useStateP(committed || '');
  // If the form is reloaded (open file / reset / rollback) the committed
  // value may change externally — re-sync the draft so the pane reflects it.
  useEffectP(() => { setDraft(committed || ''); }, [committed]);
  ...
  const handleChange = (e) => {
    ...
    // Hard stop — block typing past the cap. Slicing keeps paste behaviour
    // safe (if someone pastes 50k chars, we accept the first 32k rather
  ...
  // Cmd/Ctrl+S inside the textarea saves the notes (and stops the file save)
  const handleKey = (e) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            <div style={{ fontWeight: 600, fontSize: 12, color: '#111827', lineHeight: 1.2 }}>
              Notes
            </div>
            <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 1 }}>
              Saved with this PowerForm
            </div>
          </div>
        </div>
        <button
          onClick={onClose}
          title="Close (Tools → Show Notes pane to reopen)"
