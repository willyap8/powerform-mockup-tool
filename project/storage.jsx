// ---------------------------------------------------------------------------
// storage.jsx — Save/Open functionality. Two flows:
//
//   1. File download / upload (.powerform.json) — primary handoff format
//      for the configuration team.
//   2. localStorage named slots (up to MAX_SLOTS) — convenience for the
//      clinician working on multiple mockups.
//
// Saved bundle ONLY includes form blocks + sticky notes (per spec). Field
// values, history, and view settings are NOT persisted.
// ---------------------------------------------------------------------------
const { useState: useStorageState, useEffect: useStorageEffect, useRef: useStorageRef } = React;
...
// ---------------------------------------------------------------------------
function serializeDesign(form, notes) {
  // Strip runtime IDs — they'll be regenerated on load to avoid collisions.
...
function deserializeDesign(raw) {
  ...
  // Re-assign IDs so dragging/selection work cleanly on load
...
// ---------------------------------------------------------------------------
function downloadDesignFile(form, suggestedName, notes) {
  const payload = serializeDesign(form, notes);
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  // Sanitize: derive a friendly default filename from the form title.
...
function openDesignFile(onLoaded, onError) {
...
function saveToSlot(name, form, notes) {
...
// Pointer to the slot that autosave writes to. Set whenever the user does
// "Save to browser…" so subsequent edits silently update the same slot.
function getCurrentSlotName() {
  try { return localStorage.getItem(CURRENT_SLOT_KEY) || null; } catch (e) { return null; }
}
function setCurrentSlotName(name) {
  try {
...
// ---------------------------------------------------------------------------
function SaveSlotModal({ form, notes, onClose, onSaved }) {
  const [name, setName] = useStorageState((form.title || '').trim() || 'My PowerForm');
  const [confirmOverwrite, setConfirmOverwrite] = useStorageState(false);
  const slots = loadSlots();
...
// ---------------------------------------------------------------------------
function OpenSlotModal({ onClose, onPick, dirty }) {
  const [slots, setSlots] = useStorageState(loadSlots());
  const [confirmDelete, setConfirmDelete] = useStorageState(null);
  const slotNames = Object.keys(slots).sort();
...
// ---------------------------------------------------------------------------
function ConfirmDiscardModal({ message, confirmLabel, onCancel, onConfirm }) {
  return (
...
Object.assign(window, {
  ...
  loadSlots, saveSlots, saveToSlot, deleteSlot, MAX_SLOTS,
  getCurrentSlotName, setCurrentSlotName, getSlot,
  SaveSlotModal, OpenSlotModal, ConfirmDiscardModal,
  AutosaveIndicator,
});

// ---------------------------------------------------------------------------
// AutosaveIndicator — text + save button shown in the top bar, right of Help.
// Re-renders every 30s so the "X min ago" string stays current.
// ---------------------------------------------------------------------------
function AutosaveIndicator({ slotName, lastSavedAt, onClickSave }) {
  // Re-render every 30s while a save exists so the relative time updates.
  const [tick, setTick] = useStorageState(0);
  useStorageEffect(() => {
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        background: slotName ? '#10b981' : '#9ca3af',
      }} />
...
// Renders "just now" / "X sec ago" / "X min ago" / "X hr ago" / "date string"
