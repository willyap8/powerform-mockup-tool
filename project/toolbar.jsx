// ---------------------------------------------------------------------------
// toolbar.jsx — Floating, draggable, minimisable element palette.
// Click a button to arm click-to-place mode. The host shows the yellow banner
// and tracks the active tool.
// ---------------------------------------------------------------------------
...
function FloatingToolbar({ activeTool, setActiveTool, disabled, leftOffset }) {
  const offset = leftOffset || 0;
  const [pos, setPos] = useStateTB({ x: 16 + offset, y: 96 });
  const [min, setMin] = useStateTB(false);
  const dragRef = useRefTB(null);

  // If the host shifts (e.g. notes pane opens) and our current x would now
  // collide with that pane, nudge ourselves out to clear it. Only pushes
  // right — user-chosen positions are otherwise preserved.
  useEffectTB(() => {
...
Object.assign(window, { FloatingToolbar });
